    ...
    [SerializeField] private List<MonsterPrefabEntry> monsterPrefabs = new();
        ...
        private void Start()
        {
            ...
            foreach (StageMonsterSpawnRule rule in stageRules)
            {
                ...
                List<StageMonsterSpawnRule> stageRules2 = StageCsvLoader.LoadStageMonsterRules(stageId);

                foreach (StageMonsterSpawnRule rules in stageRules)
                {
                    GameObject prefabs = Resources.Load<GameObject>(rules.MonsterId);
                    if (prefabs == null)
                    {
                        prefabs = Resources.Load<GameObject>($"Prefabs/{rule.MonsterId}");
                    }
                    ...
                    SpawnRuntimeState state = new SpawnRuntimeState(rule, prefabs);
